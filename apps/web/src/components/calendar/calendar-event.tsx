...
export default function CalendarEvent({
  ...
  const { events, date, getColorByEvent, service, dialogControl } =
    useCalendarContext()
  const style = month ? {} : calculateEventPosition(event, events)

  // Get event color using service or custom function
  const eventColor = getColorByEvent 
    ? getColorByEvent(event)
    : service?.getColorByEvent(event) || event.color
  ...
  return (
    <MotionConfig reducedMotion="user">
      <AnimatePresence mode="wait">
        <motion.div
          className={cn(
            `px-3 py-1.5 rounded-md truncate cursor-pointer transition-all duration-300 bg-${eventColor}-500/10 hover:bg-${eventColor}-500/20 border border-${eventColor}-500`,
            !month && 'absolute',
            className
          )}
          style={style}
          onClick={(e) => {
            e.stopPropagation()
            dialogControl.show({ mode: 'edit', eventId: event.id })
          }}
          initial={{
            opacity: 0,
            y: -3,
        ...
        >
          <motion.div
            className={cn(
              `flex flex-col w-full text-${eventColor}-500`,
